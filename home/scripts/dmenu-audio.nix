...
pkgs.writeShellApplication {
  ...
  text =
    ...
    in
    ''
      sinkname=$(${pw-dump} | ${jq} -r '.[] | select(.info.props."media.class" == "Audio/Sink") | [.info.props."node.name", .info.props."node.description"] | join("\t")' | grep -vE "HDMI|DisplayPort" | sort -u -k2 | ${fuzzel} --dmenu --with-nth=2 --accept-nth=1 --lines=5)
